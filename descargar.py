...
# Modificado para aceptar solo archivos .txt,preguntar por directorio de almacenamiento, y contar urls descargadas 
...
def main():
    ...
    input_file = input("Ingrese el nombre del archivo que contiene los enlaces (debe ser .txt): ").strip()
    ...
    try:
        ...
        file_mime_type = mime.from_file(input_file)
        
        if file_mime_type != 'text/plain':
            print("Error: El archivo debe ser de tipo texto plano (text/plain)")
            return
        
    except Exception as e:
        print(f"Error al verificar el tipo MIME: {str(e)}")
        return

    # Directorio donde se guardarán los archivos descargados
    download_path = input("Ingrese el directorio donde guardará los archivos: ")
    ...
    try:
        ...
        # Contadores para URLs procesadas y descargadas
        total_urls = 0
        downloaded_urls = 0
        ...
        for index, url in enumerate(urls, start=1):
            ...
            if url:  # Verificar que la URL no esté vacía
                total_urls += 1
                if download_file(url, download_path, index):
                    downloaded_urls += 1
        
        # Mostrar resumen de descargas
        print(f"\nProceso completado. URLs procesadas: {total_urls}")
        print(f"URLs descargadas exitosamente: {downloaded_urls}")
        if total_urls > 0 and downloaded_urls < total_urls:
            print(f"URLs fallidas: {total_urls - downloaded_urls}")
